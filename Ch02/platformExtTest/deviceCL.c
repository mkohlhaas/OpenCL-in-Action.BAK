...
GArray *getDeviceFPConfig(cl_device_id device, cl_device_info paramName) {
  GArray *fpConfigs = g_array_new(FALSE, FALSE, sizeof(property));
  size_t paramSize;
  cl_int err = clGetDeviceInfo(device, paramName, 0, NULL, &paramSize);
  handleError(err, "Couldn't get device atomic capabilities.");
...
GArray *getDeviceSingleFPConfig(cl_device_id device) { return getDeviceFPConfig(device, CL_DEVICE_SINGLE_FP_CONFIG); }
GArray *getDeviceDoubleFPConfig(cl_device_id device) { return getDeviceFPConfig(device, CL_DEVICE_DOUBLE_FP_CONFIG); }
...
GArray *getDeviceAtomicCapabilities(cl_device_id device, cl_device_info paramName) {
...
GArray *getDeviceAtomicMemoryCapabilities(cl_device_id device) {
  return getDeviceAtomicCapabilities(device, CL_DEVICE_ATOMIC_MEMORY_CAPABILITIES);
}

GArray *getDeviceAtomicFenceCapabilities(cl_device_id device) {
  return getDeviceAtomicCapabilities(device, CL_DEVICE_ATOMIC_FENCE_CAPABILITIES);
}
...
GArray *getDeviceQueueProperties(cl_device_id device, cl_device_info paramName) {
  GArray *queueProperties = g_array_new(FALSE, FALSE, sizeof(property));
  size_t paramSize;
  cl_int err = clGetDeviceInfo(device, paramName, 0, NULL, &paramSize);
  handleError(err, "Couldn't get device queue properties.");
...
GArray *getDeviceQueueOnDeviceProperties(cl_device_id device) { return getDeviceQueueProperties(device, CL_DEVICE_QUEUE_ON_DEVICE_PROPERTIES); }
GArray *getDeviceQueueOnHostProperties(cl_device_id device) { return getDeviceQueueProperties(device, CL_DEVICE_QUEUE_ON_HOST_PROPERTIES); }
...
cl_uint                  getDeviceNumericVersion(cl_device_id device)                      { return getDeviceUint(device,       CL_DEVICE_NUMERIC_VERSION,                         "Couldn't get numeric version."); }
...
cl_ulong                 getDeviceMaxMemAllocSize(cl_device_id device)                     { return getDeviceUlong(device,      CL_DEVICE_MAX_MEM_ALLOC_SIZE,                      "Couldn't get max mem alloc size."); }
cl_ulong                 getDeviceGlobalMemCacheSize(cl_device_id device)                  { return getDeviceUlong(device,      CL_DEVICE_GLOBAL_MEM_CACHE_SIZE,                   "Couldn't get global mem cache size."); }
cl_ulong                 getDeviceGlobalMemSize(cl_device_id device)                       { return getDeviceUlong(device,      CL_DEVICE_GLOBAL_MEM_SIZE,                         "Couldn't get global mem size."); }
cl_ulong                 getDeviceMaxConstantBufferSize(cl_device_id device)               { return getDeviceUlong(device,      CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,                "Couldn't get max constant buffer size."); }
cl_ulong                 getDeviceLocalMemSize(cl_device_id device)                        { return getDeviceUlong(device,      CL_DEVICE_LOCAL_MEM_SIZE,                          "Couldn't get local mem size."); }
