...
GArray *getDeviceQueueProperties(cl_device_id device, cl_device_info paramName) {
  ...
  cl_command_queue_properties cmdQueueProperty;
  err = clGetDeviceInfo(device, paramName, paramSize, &cmdQueueProperty, NULL);
  handleError(err, "Couldn't get device queue properties.");

  {
    property queueProperty = {FALSE, "out_of_order_exec_mode_enable"};
    if (cmdQueueProperty & CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE) {
...

  {
    ...
    if (cmdQueueProperty & CL_QUEUE_PROFILING_ENABLE) {
      queueProperty.enabled = TRUE;
    }
    g_array_append_val(queueProperties, queueProperty);
  }

  {
    ...
    if (cmdQueueProperty & CL_QUEUE_ON_DEVICE) {
      queueProperty.enabled = TRUE;
    }
    g_array_append_val(queueProperties, queueProperty);
  }

  {
    ...
    if (cmdQueueProperty & CL_QUEUE_ON_DEVICE_DEFAULT) {
      queueProperty.enabled = TRUE;
    }
    g_array_append_val(queueProperties, queueProperty);
  }
